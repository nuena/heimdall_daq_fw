...
class outShmemIface():
    ...
    def __init__(self, shmem_name, shmem_size, drop_mode = False):
        ...
        try:
            ...
            self.logger.warning("Shared memory "+ shmem_name + "_A does not exist (info, not error)")
        try:
            shmem_B = shared_memory.SharedMemory(name=shmem_name+'_B',create=False, size=shmem_size)
            shmem_B.close()
            #shmem_B.unkink()
        except FileNotFoundError as err:
            self.logger.warning("Shared memory "+ shmem_name + "_B does not existi (info, not error)")
