...
{     
    ...
    log_debug("[rtlsdrCallback] Read at device:%d, buff index:%llu, write index:%d",rtl_rec->dev_ind, rtl_rec->buff_ind, wr_buff_ind);
    rtl_rec->buff_ind++;
...
{   
    ...
    */
    ...
    log_info("Log level is set to %d", config.log_level); 
    ...
    log_debug("Initialization done. Starting main loop"); 
    while( !exit_flag )
    {   
         ...
         */        
        log_trace("Begin of main loop"); 
        pthread_cond_wait(&buff_ind_cond, &buff_ind_mutex);
        log_trace("Wait for MUTEX done"); 
        data_ready = 1;
        for(int i=0; i<ch_no; i++)
        {
            ...
            {
                log_trace("Read Buffer index issue (?) rtl_rec->buff_ind is %d, max acceptable is %d -> exiting main loop", rtl_rec->buff_ind, read_buff_ind); 
                data_ready = 0; 
                break;
            }      
        }               
        if (data_ready == 1)
        {
            log_trace("Data is ready - filling IQ Header"); 
            /*
             *---------------------
             *  Complete IQ header 
             *---------------------
            ...
            /* Sending IQ header */
            log_trace("Sending IQ Header to stdout"); 
            ...
            {   
                log_trace("Sending data to stdout. %d channels, %d samples", ch_no, buffer_size / 2); 
